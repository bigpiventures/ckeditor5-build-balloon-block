...
import AnnotateTableCommand from './commands/annotatetablecommand';
...
export default class TableEditing extends Plugin {
	...
	init() {
		...
		schema.register( 'table', {
			allowWhere: '$block',
			allowAttributes: [ 'headingRows', 'headingColumns', 'tableType' ],
			isLimit: true,
			isObject: true
		} );
		...
		// Table annotation conversion
		// const annotateTableOptions = editor.config.get( 'annotateTable.options' );
		...
		conversion.for( 'downcast' ).add( dispatcher => {
			dispatcher.on( 'attribute:tableType', ( evt, data, conversionApi ) => {
				...
				if ( viewElement && viewElement.childCount === 2 ) {
					// figure > selector, table
					viewElement = viewElement.getChild( 1 );
				} else if ( viewElement && viewElement.childCount === 1 ) {
		...
		editor.commands.add( 'annotateTable', new AnnotateTableCommand( editor ) );
...
// function _buildTableAnnotationConversionConfig( options ) {
// 	const definition = {
// 		model: {
// 			key: 'tableType',
// 			values: []
...
// 		definition.model.values.push( option.value );
// 		definition.view[ option.value ] = {
// 			name: 'table',
// 			key: 'class',
// 			value: `ck-annotated-table--${ option.value } ck-annotated-table`
// 		};
// 	}
// 	return definition;
// }
