...
export default function upcastTable() {
	return dispatcher => {
		dispatcher.on( 'element:table', ( evt, data, conversionApi ) => {
			...
			const { rows, headingRows, headingColumns, tableType } = scanTable( viewTable );
			...
			if ( tableType ) {
				attributes.tableType = tableType;
			}
...
function scanTable( viewTable ) {
	...
	const classNames = viewTable.getClassNames();
	for ( const className of classNames ) {
		if ( className.indexOf( 'ck-annotated-table' ) > -1 && className.length > 18 ) {
			const tableType = className.split( '--' )[ 1 ];
			if ( tableType ) {
